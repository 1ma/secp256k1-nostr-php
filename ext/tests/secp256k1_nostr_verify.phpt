...
var_dump(secp256k1_nostr_verify(hex2bin($publicKey), hex2bin($message), hex2bin($sig_ok)));
var_dump(secp256k1_nostr_verify(hex2bin($publicKey), hex2bin($message), hex2bin($sig_bad)));
?>
--EXPECT--
bool(true)
bool(false)
