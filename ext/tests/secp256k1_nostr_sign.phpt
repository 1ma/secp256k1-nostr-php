...
$signature = secp256k1_nostr_sign(hex2bin($privateKey), hex2bin($message));
var_dump(strlen($signature));
var_dump(secp256k1_nostr_verify(hex2bin($publicKey), hex2bin($message), $signature));

$signature2 = secp256k1_nostr_sign(hex2bin($privateKey), hex2bin($message));
var_dump(strlen($signature2));
var_dump(secp256k1_nostr_verify(hex2bin($publicKey), hex2bin($message), $signature2));
